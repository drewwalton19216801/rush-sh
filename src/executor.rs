    ...
    match unsafe { fork() } {
        Ok(ForkResult::Child) => {
            ...
            if !redirections.is_empty() {
                ...
                // Apply each redirection in order
                for redir in &redirections {
                    match redir {
                        FdRedirection::ToFile { fd, filename } => {
                            let expanded_filename = expand_variables_in_string(filename, &mut subshell_state);
...
mod tests {
    ...
    #[test]
    fn test_fd_redirection_duplicate() {
        let cmd = ShellCommand {
            args: vec!["sh".to_string(), "-c".to_string(), "echo test".to_string()],
            input: None,
            output: None,
